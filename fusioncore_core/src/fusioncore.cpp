...
bool FusionCore::try_fuse_gps_rotation_heading(
  ...
  const sensors::GnssPosNoiseMatrix & R_meas)
{
  if (!config_.gps_rotation_heading_enabled || fix.lever_arm.is_zero()) {
    return false;
  }
  ...
  if (std::abs(delta_yaw) < config_.gps_rotation_heading_min_yaw_delta) {
    return false;
  }
  ...
  if (arc_len < config_.gps_rotation_heading_min_arc_baseline) {
    return false;
  }
